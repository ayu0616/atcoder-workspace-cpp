...
struct State
{
    ...
    vector<State> find_nexts()
    {
        ...
        rep(i, m)
        {
            if (i == min_i)
                continue;
            State next = {b, score, n_move, {0, 0}, carried};
...
int main()
{
    ...
    int carried = 0;
    State cur = {b, 10000, 5000, {0, 0}, 0};
    while (carried < n)
    {
        cur = cur.transition();
        if (cur.move_param.v != 0)
            cout << cur.move_param.v << " " << cur.move_param.i + 1 << endl;
        cout << cur.carried << " " << 0 << endl;
        carried++;
    }
    debug(n_move);
    debug(cur.score);
