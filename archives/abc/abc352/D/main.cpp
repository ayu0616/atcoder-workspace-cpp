...
S op(S a, S b) {
    ...
    while (ai < a.size() && bi < b.size()) {
        pll p;
        if (a[ai].second + 1 == b[bi].first) {
            p = {a[ai].first, b[bi].second};
            ai++;
            bi++;
        } else if (b[bi].second + 1 == a[ai].first) {
            p = {b[bi].first, a[ai].second};
            ai++;
            bi++;
...
int main() {
    ...
    set<ll> seeing;
    while (l < r && l < N - K) {
        while (r < N && uf.size(P[r - 1]) < K) {
            seeing.insert(P[r - 1]);
            if (seeing.count(P[r - 1] - 1)) uf.link(P[r - 1], P[r - 1] - 1);
            if (seeing.count(P[r - 1] + 1)) uf.link(P[r - 1], P[r - 1] + 1);
            r++;
        }
        if (uf.size(P[l]) >= K) {
        ...
        if (seeing.count(P[l] - 1)) uf.cut(P[l], P[l] - 1);
        if (seeing.count(P[l] + 1)) uf.cut(P[l], P[l] + 1);
        seeing.erase(P[l]);
        l++;
    }
    // l = N - 2, r = N - 1;
    // while (K < r) {
    //     while (l >= 0 && uf.size(P[l]) < K) {
    //         seeing.insert(P[l]);
    //         if (seeing.count(P[l] - 1)) uf.link(P[l], P[l] - 1);
    //         if (seeing.count(P[l] + 1)) uf.link(P[l], P[l] + 1);
    //         l--;
    //     }
    //     if (uf.size(P[r]) >= K) {
    //         chmin(ans, r - l - 1);
    //     }
    //     if (seeing.count(P[r] - 1)) uf.cut(P[r], P[r] - 1);
    //     if (seeing.count(P[r] + 1)) uf.cut(P[r], P[r] + 1);
    //     seeing.erase(P[r]);
    //     r--;
    // }
